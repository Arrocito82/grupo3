function fetchLista(e) { current_list = e; var t = new XMLHttpRequest;
    t.onreadystatechange = function() { if (4 == this.readyState && 200 == this.status) { let e = JSON.parse(this.responseText); if (draggable_list.innerHTML = "", p.style.display = "none", e.length > 0) { for (let t = 0; t < e.length; t++) { let n = document.createElement("li"); const d = e[t];
...
function actualizar() { let e = new XMLHttpRequest; let t = JSON.stringify({ crud: "update", lista_id: current_list, fuente_id: fuente_id, destino: end_index });
    e.onreadystatechange = function() { if (4 == this.readyState && 200 == this.status) { let e = +this.responseText;
            1 == e && update_mesaje() } }, e.open("POST", "crud_lista.php", !0), e.setRequestHeader("Content-type", "application/json"), e.send(t) }

function eliminar() { let e = new XMLHttpRequest,
        t = document.querySelectorAll("#delete_ids"),
        n = [];
    t.forEach(e => { e.checked && n.push(e.value) }); let d = JSON.stringify({ crud: "delete", lista_id: current_list, audios_id: n });
...
function mover() { let e = document.querySelector("#draggable-list");
    end_index < index ? e.insertBefore(e.childNodes[index], e.childNodes[end_index]) : end_index > index && (end_index += 1, e.insertBefore(e.childNodes[index], e.childNodes[end_index])); let t = document.querySelectorAll("#draggable-list li"); for (let e = 0; e < t.length; e++) { let n = document.createAttribute("data-index");
        n.value = e, t[e].removeAttribute("data-index"), t[e].setAttributeNode(n) } }

function addEventListeners() { let e = document.querySelectorAll(".draggable-list li");
